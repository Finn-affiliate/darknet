...
async def alias_cmd(interaction: discord.Interaction, name: str):
    ...
    if CH_LOG:
        ch = bot.get_channel(CH_LOG)
        if ch:
            embed = discord.Embed(title="🆕 Neuer Alias-Antrag", color=0xFF8C00, timestamp=datetime.now())
            embed.add_field(name="Alias", value=f"`{name}`", inline=True)
            embed.add_field(name="User", value=f"{interaction.user.mention} (`{interaction.user.id}`)", inline=True)
            embed.set_footer(text=f"Freischalten: /freischalten @{interaction.user.name} 0")
