...
async def bewerben(interaction: discord.Interaction):
    ...
    try:
        ...
        await log_msg(f"Neue Bewerbung von `{row[0]}`", embed=embed)
        try:
            await msg.delete()
        except:
            pass
...
async def inserat(interaction: discord.Interaction, titel: str, beschreibung: str, preis: int):
    ...
    await interaction.response.send_message("✅ Inserat erstellt.", ephemeral=True)
...
async def kopfgeld(interaction: discord.Interaction, ziel: str, betrag: int):
    ...
    alias = get_alias(uid)[0]
    con = db()
    if not con.execute("SELECT 1 FROM aliases WHERE alias=? AND zugelassen=1", (ziel,)).fetchone():
    ...
    if CH_KOPFGELDER:
        ...
        if ch:
            await ch.send(embed=embed)
    await interaction.response.send_message("✅ Kopfgeld gesetzt.", ephemeral=True)
...
async def enttarnen(interaction: discord.Interaction, alias: str):
    ...
    await interaction.response.send_message(
        f"✅ Anfrage eingegangen. ${ENTTARNEN_PREIS:,} werden ingame verrechnet.\nDu wirst kontaktiert.",
        ephemeral=True
    )
    await log_msg(f"⚠️ Enttarnen-Anfrage: `{anfragen_alias}` will `{alias}` enttarnen")
